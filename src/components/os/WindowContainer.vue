  ...
  >
    ...
    >
      <div class="font-bold text-white flex items-center space-x-2">
        <div class="w-4 h-4 bg-psy-violet shadow-[0_0_10px_rgba(132,59,253,0.5)]"></div>
        <span class="uppercase font-mono tracking-widest truncate">{{ title }}</span>
      </div>
      
      <div class="flex items-center space-x-2 shrink-0" @mousedown.stop @touchstart.stop>
        <!-- Fullscreen Toggle -->
        <button 
          @click="toggleMaximize" 
          class="w-8 h-8 bg-white/10 border border-white/20 text-white rounded-full flex items-center justify-center hover:bg-psy-violet hover:text-white transition-all cursor-pointer group"
          title="Toggle Fullscreen"
        >
          <div v-if="!isMaximized" class="w-3 h-3 border border-current rounded-sm"></div>
          <div v-else class="relative w-3 h-3">
            <div class="absolute top-0 right-0 w-2 h-2 border border-current rounded-sm"></div>
            <div class="absolute bottom-0 left-0 w-2 h-2 border border-current rounded-sm bg-white/20"></div>
          </div>
        </button>
        ...
        <button 
          @click="closeWindow" 
          class="w-8 h-8 bg-white/10 border border-white/20 text-white rounded-full flex items-center justify-center hover:bg-red-500 hover:text-white transition-all cursor-pointer"
    ...
    >
      <button 
        @click="closeWindow" 
        class="w-6 h-6 bg-white/10 hover:bg-red-500 rounded-full flex items-center justify-center text-[10px] transition-all z-[60]"
        @mousedown.stop
    ...
    <div 
      ...
      :class="[isTerminal ? 'p-0 bg-transparent' : 'p-6 bg-black/20']"
    >
      <slot />
    </div>
  </div>
